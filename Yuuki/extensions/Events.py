...
from ..utils import get_attachment, find_content, send_message, PINK, CYAN
...
async def msg_delete(event: hikari.MessageDeleteEvent) -> None:
    ...
    embed = hikari.Embed(title="Message deleted!", description="A message you had been tracking has been deleted!", colour=PINK).add_field("Message content", find_content(event)).add_field("Message Author", event.old_message.author if event.old_message else "Could not find author")
...
async def message_update(event: hikari.GuildMessageUpdateEvent) -> None:
    ...
    embed = hikari.Embed(title="Message Edited!", description="A message you had been tracking has been edited!", colour=PINK).add_field("Author", event.message.author.username).add_field("Link", f"[Go to message]({event.message.make_link(event.message.guild_id)})")
...
async def reaction_add(event: hikari.GuildReactionAddEvent) -> None:
    ...
    embed = hikari.Embed(title="Reaction Added!", description="A message you had been tracking has been reacted to!", colour=CYAN).add_field("Server", guild.name).add_field("Channel", f"<#{event.channel_id}>").add_field("Emoji", emoji).add_field("Message", f"[Go to message](https://discord.com/channels/{event.guild_id}/{event.channel_id}/{event.message_id})").set_thumbnail(guild.icon_url if guild.icon_url else None)
...
async def member_remove(event: hikari.MemberDeleteEvent) -> None:
    ...
    if not s: return
    guild = await events.bot.getch_guild(event.guild_id)
    embed = hikari.Embed(title="User Removed!", description="A user you had been tracking has been removed from a server!", colour=CYAN).add_field("Server", guild.name).add_field("User", str(event.user)).set_thumbnail(event.user.default_avatar_url if event.user.default_avatar_url else None)
...
async def member_create(event: hikari.MemberCreateEvent) -> None:
    ...
    embed = hikari.Embed(title="User Joined!", description="A user you had been tracking has joined a new server!", colour=CYAN).add_field("Server", guild.name).add_field("User", str(event.user)).set_thumbnail(event.member.display_avatar_url if event.member.display_avatar_url else None)
...
async def member_update(event: hikari.MemberUpdateEvent) -> None:
    ...
    embed = hikari.Embed(title="User Updated!", description="A user you had been tracking has just been updated in a server!", colour=CYAN).add_field("Server", guild.name).add_field("User", str(event.user)).set_thumbnail(event.member.display_avatar_url if event.member.display_avatar_url else None)
